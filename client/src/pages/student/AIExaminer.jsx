...
const handleSubmit = async () => {
  try {
    ...
    const response = await apiClient.post(
      "/examiner/exam/submitOmr",
      formData
    );

    if (response.data.success) {
      toast.success("OMR uploaded successfully");

      const submissionId =
        response.data.submission?._id || response.data.submissionId;
      ...
      navigate(`/ai-examiner/detected/${submissionId}`);
    }
  } catch (err) {
    toast.error(err.response?.data?.message || "Failed to upload OMR");
  }
};
